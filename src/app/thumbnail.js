...
export default function Thumbnail({ item, allowCycling = false, onImageClick, priority = false }) {
  ...
  const [availableImages, setAvailableImages] = useState([`/images/${item.id}.jpg?${cacheVersion}`]);
  ...
  useEffect(() => {
    ...
    setAvailableImages([`/images/${item.id}.jpg?${cacheVersion}`]);
  ...
  useEffect(() => {
    ...
    const discoverImages = async () => {
      ...
      const probes = [];
      for (let i = 1; i <= 10; i++) {
        const path = `/images/${item.id}-${i}.jpg?${cacheVersion}`;
